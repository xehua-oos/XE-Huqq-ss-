...
function Aimbot.ensureDrawings()
    if not Aimbot.drawings.mouseFovCircle then
        ...
        if success then
            ...
            obj.Filled = false; obj.Radius = Aimbot.mouseFOV; obj.Visible = Aimbot.showMouseFOV and Aimbot.enabled
            obj.ZIndex = 10; Aimbot.drawings.mouseFovCircle = obj
        end
    end
    if not Aimbot.drawings.fixedFovCircle then
        ...
        if success then
            ...
            obj.Filled = false; obj.Radius = Aimbot.fixedFOV; obj.Visible = Aimbot.showFixedFOV and Aimbot.enabled
            obj.ZIndex = 9; Aimbot.drawings.fixedFovCircle = obj
        end
    end
    if not Aimbot.drawings.aimLine then
        ...
        if success then
            ...
            obj.Visible = Aimbot.lineEnabled and Aimbot.enabled; obj.ZIndex = 11
...
function Aimbot.start()
    ...
    local function updateFixedCenter()
        local center = Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y/2)
        if Aimbot.drawings.fixedFovCircle then Aimbot.drawings.fixedFovCircle.Position = center end
        if Aimbot.drawings.aimLine then Aimbot.drawings.aimLine.From = center end
    end
...
AimbotTab:Slider({ Title = "平滑系数", Value = {Min=0, Max=1, Default=0.35}, Callback = function(v) Aimbot.smooth = v end})
AimbotTab:Slider({ Title = "预测时间", Value = {Min=0, Max=1, Default=0.12}, Callback = function(v) Aimbot.prediction = v end})
...
local function UpdateESP()
    if not ESP_ENABLED then
        -- 如果ESP被禁用，但缓存中还有对象，确保它们不可见（理论上不会发生，因为禁用时会调用ClearESP，但为了保险）
...
-- ===================== 启动完成通知 =====================
Notify("坤坤大帝脚本", "ESP彻底修复版加载成功！未开启透视时无任何显示。", 5)
