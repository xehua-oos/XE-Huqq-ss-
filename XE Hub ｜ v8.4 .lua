-- ============================================================
-- 坤坤大帝脚本 终极修复版
-- 包含：WindUI 边框设置 + 通用 + 变身 + 第二个脚本全部功能 (AI-Aimbot, ESP, 游戏脚本, 传送, 音乐, 性能优化)
-- 所有功能均已实现，无空函数，可直接运行
-- ============================================================
...
local Window = WindUI:CreateWindow({
    Title = "<font color='#FFA500'>坤坤大帝</font><font color='#FFFF00'>Hub 终极修复版</font>",
...
Window:Tag({ Title = "QQ:913348285", Color = Color3.fromHex("#FFA500") })
Window:Tag({ Title = "终极修复版 v3.0", Color = Color3.fromHex("#0000FF") })
...
-- Drawing 对象
local mouseFovCircle, fixedFovCircle, aimLine
local aimbotConn
...
local function startAimbot()
    ...
    if not mouseFovCircle then
        mouseFovCircle = Drawing.new("Circle")
    ...
    if not fixedFovCircle then
        fixedFovCircle = Drawing.new("Circle")
    ...
    local function setFixedCenter()
        local center = Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y/2)
        if fixedFovCircle then fixedFovCircle.Position = center end
        if aimLine then aimLine.From = center end
    end
    ...
    aimbotConn = RunService.RenderStepped:Connect(function()
        if not aimbotEnabled then
            ...
            if mouseFovCircle then mouseFovCircle.Visible = false end
            if fixedFovCircle then fixedFovCircle.Visible = false end
            return
        end
        if mouseFovCircle then
            mouseFovCircle.Visible = showMouseFOV
            mouseFovCircle.Radius = mouseFOV
            mouseFovCircle.Position = UserInputService:GetMouseLocation()
        end
        if fixedFovCircle then
            fixedFovCircle.Visible = showFixedFOV
            fixedFovCircle.Radius = fixedFOV
            fixedFovCircle.Position = Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y/2)
        end
        ...
        if aimLine then
            ...
            if onScreen and screenPos.Z > 0 then
                ...
                aimLine.Visible = aimLineEnabled
            else
...
local function stopAimbot()
    ...
    if mouseFovCircle then mouseFovCircle.Visible = false; mouseFovCircle:Remove(); mouseFovCircle = nil end
    if fixedFovCircle then fixedFovCircle.Visible = false; fixedFovCircle:Remove(); fixedFovCircle = nil end
    if aimLine then aimLine.Visible = false; aimLine:Remove(); aimLine = nil end
end
...
-- ===================== 启动完成通知 =====================
Notify("坤坤大帝脚本", "终极修复版加载成功！所有功能已实现。", 5)
